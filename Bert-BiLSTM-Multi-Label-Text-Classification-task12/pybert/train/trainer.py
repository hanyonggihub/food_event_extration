...
class Trainer(object):
    ...
    def valid_epoch(self,data):
        ...
        if self.epoch_metrics:
            for metric in self.epoch_metrics:
                ...
                value = metric.value()
                # print("valid_epoch："+str(value))
                if value:
                    self.result[f'valid_{metric.name()}'] = value
        if 'cuda' in str(self.device):
            torch.cuda.empty_cache()
        print("val_result："+str(self.result))
        return self.result
    ...
    def train_epoch(self,data):
        ...
        for step,  batch in enumerate(data):
            ...
            label_ids=Variable(label_ids.cpu().float(), requires_grad=True)  # 生成变量
            # logits = Variable(input=logits, requires_grad=True)
            # label_ids = torch.LongTensor(label_ids)#Variable(, requires_grad=True)
        ...
        if "cuda" in str(self.device):
            torch.cuda.empty_cache()
        print("train_result：" + str(self.result))
        return self.result
