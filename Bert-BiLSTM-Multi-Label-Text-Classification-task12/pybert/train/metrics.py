...
class AccuracyThresh(Metric):
    ...
    def value(self):
        ...
        acc_test1=np.sum(((self.y_pred>self.thresh)==self.y_true.byte()).int().cpu().numpy().tolist(), axis=1)
        print()
        print("acc_test1："+str(acc_test1))
        acc_list=[]
        acc_sum=0
        for this_acc in acc_test1:
            if(this_acc==2):
                acc_list.append(1)
                acc_sum=acc_sum+1
            else:acc_list.append(0)
        print("acc_list："+str(acc_list))
        acc=acc_sum/data_size
        #输出对应的((self.y_pred>self.thresh)==self.y_true.byte()).float().cpu().numpy()
        #能知道到底对不对，再人为去和text对应
        # TorF=((self.y_pred>self.thresh)==self.y_true.byte()).int().numpy().tolist()
        # output_onehot=(self.y_pred>self.thresh).int().numpy().tolist()
        # target_onehot=self.y_true.byte().int().numpy().tolist()
        # print("output_onehot："+str(output_onehot))
        # print("target_onehot："+str(target_onehot))
        #对比结果，输出到csv里 只有train
        # with open('TorF_bitch.csv', "a+", newline='') as file:  # 处理csv读写时不同换行符  linux:\n    windows:\r\n    mac:\r
        #     csv_file = csv.writer(file,dialect='excel')
        #     TorF.append([-1, -1, -1, -1, -1, -1, -1])
        #     datas =TorF
        #     csv_file.writerows(datas)
        # with open('check_target_bitch.csv', "a+", newline='') as file:  # 处理csv读写时不同换行符  linux:\n    windows:\r\n    mac:\r
        #     csv_file = csv.writer(file,dialect='excel')
        #     target_onehot.append([-1, -1, -1, -1, -1, -1, -1])
        #     datas =target_onehot
        #     csv_file.writerows(datas)
        # with open('check_output_bitch.csv', "a+", newline='') as file:  # 处理csv读写时不同换行符  linux:\n    windows:\r\n    mac:\r
        #     csv_file = csv.writer(file,dialect='excel')
        #     output_onehot.append([-1, -1, -1, -1, -1, -1, -1])
        #     datas =output_onehot
        #     csv_file.writerows(datas)
        # print(acc/data_size)
        return acc
...
class AUC(Metric):
    ...
    def value(self):
        ...
        # print("auc："+str(auc))
        data_size = self.y_true.shape[0]
        if torch.is_tensor(self.y_prob):
            self.y_prob=self.y_prob.numpy()
        acc_test1 = np.sum(((self.y_prob > 0.5) == self.y_true.byte().numpy()).astype(int).tolist(), axis=1)
        # print()
        print("acc_test1：" + str(acc_test1))
        acc_list = []
        acc_sum = 0
        for this_acc in acc_test1:
            if (this_acc == 2):
                ...
                acc_list.append(0)
        print("acc_list：" + str(acc_list))
        # TorF = ((self.y_prob >0.5) == self.y_true.byte()).int().numpy().tolist()
        # output_onehot = (self.y_prob > 0.5).astype(int).tolist()
        # target_onehot = self.y_true.byte().numpy().astype(int).tolist()
        # print("output_onehot："+str(output_onehot))
        # print("target_onehot："+str(target_onehot))
        # 对比结果，输出到csv里
        # with open('TorF_bitch.csv', "a+", newline='') as file:  # 处理csv读写时不同换行符  linux:\n    windows:\r\n    mac:\r
        ...
        #     datas =TorF
        #     csv_file.writerows(datas)
        # with open('check_target_epoch.csv', "a+", newline='') as file:  # 处理csv读写时不同换行符  linux:\n    windows:\r\n    mac:\r
        #     csv_file = csv.writer(file, dialect='excel')
        #     target_onehot.append([-1, -1, -1, -1, -1, -1, -1])
        #     datas = target_onehot
        #     csv_file.writerows(datas)
        # with open('check_output_epoch.csv', "a+", newline='') as file:  # 处理csv读写时不同换行符  linux:\n    windows:\r\n    mac:\r
        #     csv_file = csv.writer(file, dialect='excel')
        #     output_onehot.append([-1,-1,-1,-1,-1,-1,-1])
        #     datas = output_onehot
        #     csv_file.writerows(datas)
