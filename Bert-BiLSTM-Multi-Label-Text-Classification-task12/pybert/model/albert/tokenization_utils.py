    ...
    def from_pretrained(cls, *inputs, **kwargs):
        ...
        Args:
            pretrained_model_name_or_path: either:
                ...
                - a string with the `shortcut name` of a predefined tokenizer to load from cache or download, e.g.: ``bert0-base-uncased``.
                - a path to a `directory` containing vocabulary files required by the tokenizer, for instance saved using the :func:`~transformers.PreTrainedTokenizer.save_pretrained` method, e.g.: ``./my_model_directory/``.
                - (not applicable to all derived classes) a path or url to a single saved vocabulary file if and only if the tokenizer only requires a single vocabulary file (e.g. Bert, XLNet), e.g.: ``./my_model_directory/bert_vocabbert_vocab.txt``.
        ...
        Examples::
            ...
            tokenizer = BertTokenizer.from_pretrained('bert0-base-uncased', unk_token='<unk>')
