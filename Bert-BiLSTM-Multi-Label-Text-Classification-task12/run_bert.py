...
from pybert.train.losses import BCEWithLogLoss,CrossEntropyLoss
import torch.nn as nn
from pybert.train.trainer import Trainer
from torch.utils.data import DataLoader
from pybert.io.utils import collate_fn
from pybert.io.bert_processor import BertProcessor
from pybert.common.tools import init_logger, logger #我没用这个
...
def run_train(args):
    ...
    #nn.CrossEntropyLoss()#BCEWithLogLoss
    trainer = Trainer(args= args,model=model,logger=logger,criterion=BCEWithLogLoss(),optimizer=optimizer,
                      ...
                      epoch_metrics=[AUC(average='micro', task_type='multiclass'),
...
def run_test(args):
    ...
    data = TaskData()
    targets, sentences,triggers = data.read_data(raw_data_path=config['test_path'],
                                        preprocessor=EnglishPreProcessor())
    print("targets0："+str(targets))
    lines = list(zip(sentences, targets,triggers))
    processor = BertProcessor(vocab_path=config['bert_vocab_path'], do_lower_case=args.do_lower_case)
    label_list = processor.get_labels()
    id2label = {i: label for i, label in enumerate(label_list)}
    ...
    predictor = Predictor(model=model,logger=logger,n_gpu=args.n_gpu,epoch_metrics=[AUC(average='micro', task_type='binary')],batch_metrics=[AccuracyThresh(thresh=0.5)],criterion=BCEWithLogLoss())
    all_onehots,texts_labels_list = predictor.predict(data=test_dataloader)
    print(all_onehots)
    print(texts_labels_list)

def main():
    parser = ArgumentParser()
    parser.add_argument("--arch", default='bert1', type=str)
    ...
    parser.add_argument("--epochs", default=3, type=int)#3--->4
    ...
    parser.add_argument("--train_max_seq_len", default=200, type=int)
    parser.add_argument("--eval_max_seq_len", default=200, type=int)
    parser.add_argument('--loss_scale', type=float, default=0)
    parser.add_argument("--warmup_proportion", default=0.1, type=float)
    parser.add_argument("--weight_decay", default=0.01, type=float)
    parser.add_argument("--adam_epsilon", default=1e-8, type=float)
    parser.add_argument("--grad_clip", default=1.0, type=float)
    parser.add_argument("--learning_rate", default=1e-5, type=float)#2e-4 3e-5 e-5
    ...
    #init_logger(log_file='C:\\Users\\23508\\Desktop\\nlp\\本科毕设\\Bert-Multi-Label-Text-Classification-master\\Bert-Multi-Label-Text-Classification-master/pybert\\output\\log\\bert1-2021-11-14-15:23:21.log')
    ...
    if args.do_data:
        from pybert.io.task_data import TaskData
        data = TaskData()
        targets, sentences,triggers = data.read_data(raw_data_path=config['raw_data_path'],
                                            preprocessor=EnglishPreProcessor())
        data.train_val_split(X=sentences, y=targets,triggers=triggers, shuffle=True, stratify=False,
