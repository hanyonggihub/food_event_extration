...
def train(hparams):
    ...
    TF_token_b=hparams.TF_token_b
    # train_dataset
    train_dataset = NerDataset(train_file, tagset_path=tagset_file,
                               pretrained_model_path=pretrained_model_path,
                               max_len=max_len, is_train=True,TF_token_b=TF_token_b)
    ...
    for epoch in range(epoch_offset, epochs):
        ...
        for i_batch, sample_batched in enumerate(tqdm(train_loader, desc='Training')):
            ...
            ner_ids = sample_batched['ner_ids'].to(device)
            model.zero_grad()
            loss = model(token_ids, tag_ids, token_type_ids, attention_mask,ner_ids=ner_ids)
            loss.backward()
            optimizer.step()
            loss_batchs.append(float(loss))##
            running_loss += loss.item()
            if i_batch % 10 == 9:
                writer.add_scalar('Training/training loss', running_loss / 10, epoch * len(train_loader) + i_batch)
                running_loss = 0.0
        with open('loss_result.txt', 'ab+') as f:
            text1 = str(sum(loss_batchs)/len(loss_batchs)) + '\n'
            print(text1)
            f.write(bytes(text1, 'utf-8'))
        if validation_file:
            ...
            with torch.no_grad():
                ...
                for val_i_batch, val_sample_batched in enumerate(tqdm(val_loader, desc='Validation')):
                    ...
                    tag_ids = val_sample_batched['tag_ids'].to(device)
                    ner_ids=val_sample_batched['ner_ids'].to(device)
                    pred_tag_ids = model.decode(token_ids, token_type_ids=token_type_ids,
                                                attention_mask=attention_mask,ner_ids=ner_ids)

                    seq_ends = attention_mask.sum(dim=1)
                    true_tag_ids = [_tag_ids[:seq_ends[i]] for i, _tag_ids in enumerate(tag_ids)]
                    batched_tags_true = [[idx2tag[tag_id] for tag_id in _tag_ids.tolist()] for _tag_ids in true_tag_ids]
                ...
                with open('acc_result.txt','ab+') as f:
                    text1="epoch:"+str(epoch)+"\n"+str(metric.classification_report(tags_true_list, tags_pred_list))+'\n'
                    f.write(bytes(text1,'utf-8'))
                ...
                with open('acc_result.txt','ab+') as f:
                    text2=str(epoch)+"f1:"+str(f1)+'\n'+str(epoch) + "precision:" + str(precision)+'\n'+str(epoch) + "recall:" + str(recall)+'\n'+str(epoch) + "accuracy:" + str(accuracy)+'\n'
                    f.write(bytes(text2,'utf-8'))
