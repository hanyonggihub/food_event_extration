...
def train(hparams):
    ...
    for epoch in range(epoch_offset, epochs):
        ...
        for i_batch, sample_batched in enumerate(tqdm(train_loader, desc='Training')):
            ...
            radical_token_ids = sample_batched['radical_token_ids'].to(device)
            radical_token_type_ids = sample_batched['radical_token_type_ids'].to(device)
            radical_attention_mask = sample_batched['radical_attention_mask'].to(device)
            tag_ids = sample_batched['tag_ids'].to(device)
            model.zero_grad()
            loss = model(token_ids, tag_ids, token_type_ids, attention_mask,radical_token_ids, radical_token_type_ids, radical_attention_mask)
            loss.backward()
            optimizer.step()
